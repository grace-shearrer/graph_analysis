...
head(graph_df)
graph_df$group <- factor(graph_df$group, levels = c("no","overweight","obese"))
summary(graph_df$X)
...
# General balloon plots
```{r}
x = 1

no_<-subset(graph_df, graph_df$group == "no")
...
f0<-lm(clustering ~ Age_c + HBA1c_c + race_eth + motion_log10 + BMI, data=dat)
summary(f0)


f1<-lm(clustering ~ Age_c + HBA1c_c + race_eth + motion_log10:group + BMI, data=dat)
summary(f1)


ggplot(dat, aes(BMI, clustering)) +
    geom_point(shape=1)+ geom_smooth(method=lm)


sggplot(dat, aes(clustering, motion_log10, group = group, color = group)) +
    geom_point(shape=1)+ geom_smooth(method=lm)
