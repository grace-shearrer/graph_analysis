...
out<-fit_function(x=warp_1[68], y = warp_1[31], z=warp_1[67], data = warp_1)
out
test.emm <- emmeans(out, ~ x*z)
test.emm
contrast(test.emm, "pairwise", adjust = "fdr")
...
```{r}
confint(aud[31:64][[1]][[1]])
aud[31:64][[1]][[2]]
aud[31:64][[1]][[3]]

contrast(aud[31:64][[1]][[3]], "pairwise")
```
...
levels(warp_1$mods)
warp_1$FrontoParietal <- NA
warp_1[warp_1$mods == 'no.FrontoParietal', "FrontoParietal"] <- 'aBMIFrontoParietal'
warp_1[warp_1$mods == 'ovFrontoParietal', "FrontoParietal"] <- 'hBMIFrontoParietal'
warp_1[warp_1$mods == 'ob.FrontoParietal', "FrontoParietal"] <- 'vhBMIFrontoParietal'
colnames(warp_1)
```
...
df_PCFP<-bind_rows(l2)
df_PCFP$pFDR<-p.adjust(df_PCFP$p.value, method = "BH")
df_PCFP[df_PCFP$p.value <= 0.05, ] 
```
#### somatosensory hand
```{r}
# View(module_df)
