...
### 3.3.1 Nodal
After correcting for multiple comparisons (n=300), there were no differences in clustering coefficient between the BMI groups. Using a less stringent correction for multiple comparison (n=3 per node), in node 23 the aBMI group exhibited higher clustering coefficient compared to the hBMI group (t-ratio = 2.84, SE=0.02, pFDR3 = 0.01). In node 74, the aBMI group has higher clustering coefficient compared to the vhBMI group (t-ratio= 2.62, SE= 0.01, pFDR3= 0.01), and the hBMI group showed increased clustering coefficient compared the the vhBMI group (t-ratio= 3.20, SE= 0.01, pFDR3= 0.004). 
...
## 3.4.1 Nodal
No differences between groups by nodal participation coefficient were observed with the more stringent (n=300) correction. Using the less stringent (n=3) correction for multiple comparisons, in node 14  the hBMI group exhibited higher participation coefficient compared to both the aBMI (t-ratio= -2.38, SE= 0.06, pFDR3= 0.03) and the vhBMI groups (t-ratio= 2.88, SE= 0.08, pFDR3= 0.01). In area 33, the hBMI again showed increased participation coefficient compared to the aBMI group (t-ratio= -2.49, SE= 0.07, pFDR3= 0.04). 
...
### 3.4.3 Module
Within the visual modules, the vhBMI showed higher participation coefficients compared to both the aBMI (t-ratio= -5.60, SE= 0.04, pFDR< 0.0001) and the hBMI groups (t-ratio=-4.98, SE= 0.04, pFDR= 0.0001). Conversely, the participation coefficient for the vhBMI was lower in the sensory module (module 4) compared to the aBMI (module 2) (t-ratio=-10.27 , SE= 0.03 pFDR< 0.0001 ) and hBMI (module 2) (t-ratio= 9.93, SE= 0.03, pFDR< 0.0001). 
## 3.6 Betweenness centrality
### 3.6.1 Nodal
No differences were detected between BMI groups after correcting for multiple comparisons (n=300). Using the less stringent multiple comparison correction (n=3),  in node 14 the aBMI group had higher centrality compared to the hBMI group (t-ratio= 2.09, SE= 0.002, pFDR3= 0.05), and the vhBMI had higher centrality compared to the hBMI group (t-ratio= -2.83, SE= 0.003, pFDR3= 0.01). In node 18, the hBMI had higher centrality compared to both the aBMI (t-ratio= -2.38, SE= 0.002, pFDR3= 0.03) and vhBMI (t-ratio= 2.44, SE= 0.002, pFDR3= 0.03) groups. In node 42, the aBMI had higher centrality compared to the hBMI (t-ratio= 2.39, SE= 0.002, pFDR3= 0.02), and the vhBMI group exhibited higher centrality compared to the hBMI group (t-ratio= -3.47, SE= 0.003, pFDR3= 0.001). In node 51, the hBMI group exhibited lower centrality compared to both the aBMI (t-ratio= 2.85, SE= 0.002, pFDR3= 0.01) and the vhBMI (t-ratio= -1.94, SE= 0.003, pFDR3= 0.08) groups. In node 87, the vhBMI group exhibited decreased centrality compared to both the aBMI (t-ratio= 2.56, SE= 0.002, pFDR3= 0.02) and the hBMI (t-ratio= 3.02, SE= 0.002, pFDR3= 0.01) groups. 
...
# x is the group
# z is the graph measures
names<-colnames(warp_1[28:64])

aud <- list()
for (i in seq(28, 64, by= 1)){
  print(paste("start", i))
  out<-fit_function(x=warp_1[70], y = warp_1[i], z=warp_1[66], data = warp_1)
...
warp_1[warp_1$mods == 'ovSmhand', "Smhand"] <- 'hBMISmhand'
...
for (i in seq(28, 64, by= 1)){
  print(paste("start", i))
  out<-fit_function(x=warp_1[69], y = warp_1[i], z=warp_1[65], data = warp_1)
...
df_PChand<-bind_rows(l2)
df_PChand$pFDR<-p.adjust(df_PChand$p.value, method = "BH")
df_PChand[df_PChand$p.value <= 0.05, ] 
```
#### somatosensory mouth 
```{r}
# View(module_df)
...
warp_1[warp_1$mods == 'ovSmmouth', "Smmouth"] <- 'hBMISmmouth'
...
# x is the group
# z is the graph measures
names<-colnames(warp_1[28:65])

l <- list()
for (i in seq(31, 65, by= 1)){
  print(paste("start", i))
  out<-fit_function(x=warp_1[69], y = warp_1[i], z=warp_1[65], data = warp_1)
  test.emm <- emmeans(out, "x")
  df<-data.frame(contrast(test.emm, "pairwise",  adjust = "fdr"))
...
l2<-list()
for(i in seq(1, 37, by = 1)){
  print(paste ("this is ",i))
  df<-l[28:65][[i]][[2]]
  df$var<-names[i]
  l2[[i]] <-list(df)
  }
df_PCmouth<-bind_rows(l2)
df_PCmouth$pFDR<-p.adjust(df_PCmouth$p.value, method = "BH")
df_PCmouth[df_PCmouth$p.value <= 0.05, ] 
```
...
# x is the group
# z is the graph measures
names<-colnames(warp_2[28:65])

l <- list()
for (i in seq(28, 65, by= 1)){
  print(paste("start", i))
  out<-fit_function(x=warp_2[69], y = warp_2[i], z=warp_2[65], data = warp_2)
  test.emm <- emmeans(out, "x")
  df<-data.frame(contrast(test.emm, "pairwise",  adjust = "fdr"))
  l[[i]] <-list(out,df)
}
...
# x is the group
# z is the graph measures
names<-colnames(warp_3[28:65])

l <- list()
for (i in seq(28, 65, by= 1)){
  print(paste("start", i))
  out<-fit_function(x=warp_3[69], y = warp_3[i], z=warp_3[66], data = warp_3)
  test.emm <- emmeans(out, "x")
  df<-data.frame(contrast(test.emm, "pairwise",  adjust = "fdr"))
  l[[i]] <-list(out,df)
}
...
df_Cenviz<-bind_rows(l2)
df_Cenviz$pFDR<-p.adjust(df_Cenviz$p.value, method = "BH")
df_Cenviz[df_Cenviz$pFDR <= 0.05, ] 
```
## Clustering coefficient 
#### Sensory 
```{r}
...
summary(as.factor(warp_X$sensory))
```
...
# x is the group
# z is the graph measures
names<-colnames(warp_X[28:65])

l <- list()
for (i in seq(28, 65, by= 1)){
  print(paste("start", i))
  out<-fit_function(x=warp_X[69], y = warp_X[i], z=warp_X[19], data = warp_X)
  test.emm <- emmeans(out, "x")
  df<-data.frame(contrast(test.emm, "pairwise",  adjust = "fdr"))
  l[[i]] <-list(out,df)
}
