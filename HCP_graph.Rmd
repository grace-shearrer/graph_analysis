...
nodes<-table(graph_df$group, graph_df$node_type)
nodes
write.table(nodes, "~/Google Drive/HCP/HCP_graph/1200/datasets/tmp/node_freq.csv", sep=",")
...
balloonplot(table(no_$module, no_$condensed), main ="Module Frequencies per functional network average BMI", xlab ="", ylab="", label = TRUE, show.margins = FALSE, colsrt = 90, text.size=x ,dotsize = 5, colmar=5, dotcolor=cbPalette[1])
...
p2<-balloonplot(table(ov_$module, ov_$area), main ="Module Frequencies per functional network high BMI", xlab ="", ylab="", label = TRUE, show.margins = FALSE, colsrt = 90, text.size=x ,dotsize = 5, colmar=5, dotcolor=cbPalette[2])
...
p3<-balloonplot(table(ob_$module, ob_$area), main ="Module Frequencies per functional network very high BMI", xlab ="", ylab="", label = TRUE, show.margins = FALSE, colsrt = 90, text.size=x ,dotsize = 5, colmar=5, dotcolor=cbPalette[3])
...
```{r}
balloonplot(table(no_$module, no_$condensed), main ="Module Frequencies per functional network average BMI", xlab ="", ylab="", label = TRUE, show.margins = FALSE, colsrt = 90, text.size=x ,dotsize = 5, colmar=5, dotcolor=cbPalette[1])
balloonplot(table(ov_$module, ov_$condensed), main ="Module Frequencies per functional network high BMI", xlab ="", ylab="", label = TRUE, show.margins = FALSE, colsrt = 90, text.size=x ,dotsize = 5, colmar=5, dotcolor=cbPalette[2])
balloonplot(table(ob_$module, ob_$condensed), main ="Module Frequencies per functional network very high BMI", xlab ="", ylab="", label = TRUE, show.margins = FALSE, colsrt = 90, text.size=x ,dotsize = 5, colmar=5, dotcolor=cbPalette[3])
```
...
```{r}
rec<-subset(graph_df, graph_df$group == "normal")
x<-table(rec$IC, rec$condensed)
colSums(x)

y<-table(rec$IC, rec$module)
...
```{r}
warp <- transform(graph_df, mods = interaction(group, condensed))

CC.gls <- gls(clustering ~ mods, data = warp)
CC.emm <- emmeans(CC.gls, "mods")
CC.fac <- update(CC.emm, levels = list(
                group = c("normal", "overweight", "obese"), 
                condensed = c('subcortical', 'sensory', 'BrainStem', 'Cerebellum',
       'CinguloOperc', 'Default', 'Attn', 'FrontoParietal',
       'MedialParietal', 'None', 'Visual')))

contrast(CC.fac, "pairwise", by = "condensed", adjust = "fdr")

DMN_PC<-subset(warp, warp$area == "Default")
violin(DMN_PC, DMN_PC$group, DMN_PC$PC)

MP_PC<-subset(warp, warp$area == "MedialParietal")
violin(MP_PC, MP_PC$group, MP_PC$PC)


describeBy(MP_PC$PC,MP_PC$group)
...
contrast(PC.fac, "pairwise", by = "condensed", adjust = "fdr")
...
total_edge<-data.frame(total)
vars<-c("contrast", "edge","t.ratio","SE", "p.value")
r2<-c("t.ratio","SE")
r3<-c("p.value")
...
keep<-c("edge","Contrast","t.ratio","SE", "p.value")   
...
write.table(all, "~/Google Drive/HCP/HCP_graph/1200/datasets/tmp/demo.csv", sep=",", row.names = F)
```
